...
public class LobbyController : MonoBehaviour
{
    ...
    {
        var newPlayerSelectTemplate = playerSelectTemplate.Instantiate();
        var newPlayerSelectController = new PlayerSelectController();

        // Assign the controller script to the visual element
        newPlayerSelectTemplate.userData = newPlayerSelectController;

        // Initialize the controller script
        newPlayerSelectTemplate.style.flexGrow = 1;
        newPlayerSelectController.CreatePlayerSelectScreen(newPlayerSelectTemplate, playableWeapons, playableCharacters, playerColors[playerPosition]);
        
        // Add the player screen to the UI document view
        _root.Q<VisualElement>("Player-" + playerPosition).Add(newPlayerSelectTemplate);
    }
}
